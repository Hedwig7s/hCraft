...
namespace hCraft {
    ...
    public enum ConfigKey {
        ...
        [StringKey( ConfigSection.General, "Custom Minecraft Server (hCraft)",
...
@"Port number on your local machine that hCraft uses to listen for
...
@"Maximum number of backup files that hCraft should keep.
...
@"hCraft includes an IRC (Internet Relay Chat) client for
...
@"IRC bot's nickname. If the nickname is taken, hCraft will append
...
Being able to receive crash reports helps identify bugs and improve hCraft!
...
Reports are confidential and are not displayed publicly." )]
        ...
        [EnumKey( ConfigSection.Advanced, hCraft.UpdaterMode.Prompt,
@"hCraft can automatically update to latest stable versions.
...
@"It is recommended to leave hCraft at default priority.
Setting this below ""Normal"" may starve hCraft of resources.
...
@"The rate at which hCraft applies block updates, in milliseconds. Lowering this will slightly
...
hCraft bind to the same IP every time." )]
        IP,

        [EnumKey( ConfigSection.Advanced, hCraft.BandwidthUseMode.Normal,
